...
export enum PlayerEventType {
	...
	QUEUE_UPDATE_EVENT = 'QueueUpdate',
	HISTORY_UPDATE_EVENT = 'HistoryUpdate'
}
...
export interface QueueUpdateEvent extends PlayerEvent {
	type: PlayerEventType.QUEUE_UPDATE_EVENT;
	size: number;
}

export interface HistoryUpdateEvent extends PlayerEvent {
	type: PlayerEventType.HISTORY_UPDATE_EVENT;
}
...
export type PlayerEvents = {
	 ...
	 * @eventProperty
	 */
	'queueUpdate': [data: QueueUpdateEvent];
	/**
	 * Emitted when the history is updated
	 * @eventProperty
	 */
	'historyUpdate': [data: HistoryUpdateEvent];
};
	...
	public onPlayerEvent(json: TrackStartEvent | TrackEndEvent | TrackStuckEvent | TrackExceptionEvent | WebSocketClosedEvent | QueueUpdateEvent | HistoryUpdateEvent): void {
		switch (json.type) {
			...
			case PlayerEventType.WEBSOCKET_CLOSED_EVENT:
				...
				break;
			case PlayerEventType.QUEUE_UPDATE_EVENT:
				this.emit('queueUpdate', json);
				break;
			case PlayerEventType.HISTORY_UPDATE_EVENT:
				this.emit('historyUpdate', json);
				break;
			default:
